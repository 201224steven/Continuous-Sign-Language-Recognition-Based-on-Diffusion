...
class BaseFeeder(data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.data_type == "video":
            ...
            # print(idx, self.inputs_list[idx]['original_info'], self.transform_mode)
            # print('len: ', len(input_data))  # 是和视频帧数一样，但slr_network中的len_x长度不对
    ...
    def read_video(self, index):  # 一次读取是读取一个视频文件夹中的所以由于拥有图像。
        # load file info
        ...
        #         img = Image.open(img_path)  # 使用opencv读取图，在训练时使用多线程会让libpng库发生错误
        #         img = np.asarray(img)
        #         imgs.append(img)
