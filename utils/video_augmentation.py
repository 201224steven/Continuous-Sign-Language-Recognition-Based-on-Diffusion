...
class TemporalRescale(object):
    ...
    def __call__(self, clip):
        ...
        if new_len <= vid_len:
            index = sorted(random.sample(range(vid_len), new_len))  # new_len 更小则对原视频进行采样
        else:
            index = sorted(random.choices(range(vid_len), k=new_len))  # new_len 更大则从原视频中可重复的选 new_len 帧组成新的视频（并保持原有帧时间的顺序）
        return clip[index]
